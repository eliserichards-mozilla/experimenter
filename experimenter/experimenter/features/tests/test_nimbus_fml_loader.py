...
class TestNimbusFmlLoader(TestCase):
    ...
    @mock.patch(
        "experimenter.features.manifests.nimbus_fml_loader.NimbusFmlLoader.get_inspector",
    )
    def test_get_fml_inspectors(self, get_inspector):
        loader = self.create_loader()
       
        clients = [mock.MagicMock()]
        result = loader.get_fml_inspectors(clients)

        get_inspector.assert_called()
        self.assertEqual(len(result), 1)

    @mock.patch(
        "experimenter.features.manifests.nimbus_fml_loader.NimbusFmlLoader.get_inspector",
    )
    def test_get_fml_inspectors_no_clients(self, get_inspector):
        loader = self.create_loader()
    ...
    @mock.patch(
        "experimenter.features.manifests.nimbus_fml_loader.NimbusFmlLoader.get_fml_clients",
    )
    @mock.patch(
        "experimenter.features.manifests.nimbus_fml_loader.NimbusFmlLoader.get_fml_inspectors",
    )
    @mock.patch(
        "experimenter.features.manifests.nimbus_fml_loader.NimbusFmlLoader.get_error",
    )
    def test_get_fml_errors(self, get_clients, get_inspectors, get_error):
        loader = self.create_loader()
        ...
        get_clients.return_value = FmlClient()
        get_inspectors.return_value = FmlFeatureInspector()
        get_error.return_value = error
        result = loader.get_fml_errors(test_blob)

        get_clients.assert_called()
        get_inspectors.assert_called_with(get_clients.return_value)
        get_error.assert_called_with(get_inspectors.return_value, test_blob)
